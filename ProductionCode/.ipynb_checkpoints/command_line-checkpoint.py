...
DEFAULT_DATA_PATH = Path(__file__).parent.parent/'ProductionCode'/'interpol_classified_stolen_art.csv'
ARTIST_DATA_PATH = Path(__file__).parent.parent/'ProductionCode'/'artists.csv'
...
def find_artwork(name_of_creator: str) -> list[str]:
    ...
    # goes through the data and finds the artworks adding to list
    for row in data:
        if row[1] == name_of_creator:
            artworks.append(row[0])
